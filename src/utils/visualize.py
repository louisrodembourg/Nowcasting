"""
Visualisation — Houston Ship Channel (Phase 1 + Phase 2).
...
GRAVITY_PATH      = Path("data/features/houston_gravity_score.parquet")
MANIFOLD_PATH     = Path("data/features/houston_manifold.parquet")
OUT_DIR           = Path("outputs/figures")
CENTER            = [29.60, -95.05]   # Houston Ship Channel
...
def map_single_day(d: date) -> Path:
    ...
    # Ensure cluster_type column exists even if all vessels are noise
    ...
    # --- Navires en mouvement (couche de fond, échantillon 2000 pts max) ----
    ...
    # --- Navires stationnaires colorés par cluster_type ---------------------
    ...
    <div style="position:fixed;bottom:30px;left:30px;z-index:999;
      ...
      <b>Houston Ship Channel — {d}</b><br><br>
    ...
    out_path = OUT_DIR / f"houston_clusters_{d.strftime('%Y_%m_%d')}.html"
    m.save(str(out_path))
...
def map_period(start: date, end: date, max_points_per_day: int = 300) -> Path:
    ...
    while d <= end:
        ...
        if parquet_path.exists():
            ...
            # Deduplicate to one position per MMSI before sampling
    ...
    if days_missing > 0:
        log.warning("%d jour(s) sans parquet dans la période (cases vides dans l'animation)", days_missing)
    ...
    # Marqueur Harvey si dans la période
    harvey = date(2017, 8, 25)
    if start <= harvey <= end:
        folium.Marker(
            location=[29.76, -95.37],
            popup=folium.Popup("<b>Hurricane Harvey</b><br>Landfall 2017-08-25", max_width=180),
            icon=folium.Icon(color="red", icon="warning-sign", prefix="glyphicon"),
            tooltip="Hurricane Harvey (2017-08-25)",
        ).add_to(m)

    folium.LayerControl().add_to(m)

    out_path = OUT_DIR / (
        f"houston_heatmap_{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.html"
    )
    m.save(str(out_path))
    log.info("Sauvegardé : %s", out_path)
    return out_path
...
# ---------------------------------------------------------------------------
# Phase 2 — Gravity score time series + manifold 2D scatter
# ---------------------------------------------------------------------------

def plot_gravity_score() -> tuple[Path, Path]:
    """
    Two matplotlib figures:
      1. Time series of gravity_score with Harvey window highlighted
      2. 2D scatter of manifold embedding (phi_1 vs phi_2) colored by gravity_score
    Returns (ts_path, scatter_path).
    """
    if not GRAVITY_PATH.exists():
        raise FileNotFoundError(f"Gravity score not found: {GRAVITY_PATH}  — run run_phase2.py first")

    df = pl.read_parquet(GRAVITY_PATH).sort("date")
    dates         = [d for d in df["date"].to_list()]
    ...
    fig.suptitle("Houston Ship Channel — Phase 2 : Gravity Score\n(Hurricane Harvey, juillet–septembre 2017)",
                 fontsize=13, fontweight="bold")

    harvey_start_dt = HARVEY_START
    harvey_end_dt   = HARVEY_END

    for ax in axes:
        ax.axvspan(harvey_start_dt, harvey_end_dt,
                   color="#FFCDD2", alpha=0.6, label="Harvey (25–31 août)")

    # Panel 1 — gravity score
    ax0 = axes[0]
    ax0.plot(dates, gravity, color="#1565C0", linewidth=1.8, zorder=3)
    ax0.fill_between(dates, gravity, alpha=0.15, color="#1565C0")
    # Highlight characteristic points
    char_dates = [d for d, c in zip(dates, is_char) if c]
    char_vals  = [g for g, c in zip(gravity, is_char) if c]
    ax0.scatter(char_dates, char_vals, color="#F44336", zorder=5,
                s=40, label="Point caractéristique")
    ax0.set_ylabel("Gravity Score [0–1]", fontsize=10)
    ax0.set_ylim(-0.05, 1.1)
    ax0.legend(fontsize=8, loc="upper left")
    ax0.grid(axis="y", alpha=0.3)

    # Panel 2 — blocked capacity
    ax1 = axes[1]
    ax1.bar(dates, capacity / 1e6, color="#FB8C00", alpha=0.7, width=0.8)
    ax1.set_ylabel("Capacité bloquée\n(Σ L×l, ×10⁶ m²)", fontsize=10)
    ax1.grid(axis="y", alpha=0.3)

    # Panel 3 — vessel count
    ax2 = axes[2]
    ...
    plt.tight_layout()
    ts_path = OUT_DIR / "houston_gravity_timeseries.png"
    fig.savefig(ts_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    log.info("Sauvegardé : %s", ts_path)

    # ── Figure 2: Manifold 2D scatter (phi_1 vs phi_2) ───────────────────────
    if len(phi_cols) < 2:
    ...
    fig2.suptitle("Houston — Espace Manifold (ϕ₁ vs ϕ₂)\ncoloré par Gravity Score",
                  fontsize=12, fontweight="bold")

    sc = ax.scatter(phi1, phi2, c=gravity, cmap="RdYlBu_r",
                    s=60, zorder=3, vmin=0, vmax=1)
    plt.colorbar(sc, ax=ax, label="Gravity Score")

    # Label Harvey days
    for i, (d, p1, p2) in enumerate(zip(dates, phi1, phi2)):
        if HARVEY_START <= d <= HARVEY_END:
    ...
    for i, (d, p1, p2, c) in enumerate(zip(dates, phi1, phi2, is_char)):
        if c and not (HARVEY_START <= d <= HARVEY_END):
            ax.annotate(d.strftime("%d/%m"), (p1, p2),
                        fontsize=7, color="#1A237E",
                        xytext=(4, 4), textcoords="offset points")

    harvey_patch = mpatches.Patch(edgecolor="#B71C1C", facecolor="none",
                                  linewidth=2, label="Harvey (25–31 août)")
    ax.legend(handles=[harvey_patch], fontsize=9)
    ...
    scatter_path = OUT_DIR / "houston_manifold_2d.png"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Visualisation Houston — Phase 1 & 2")
    ...
    parser.add_argument("--end", metavar="YYYY-MM-DD",
                        help="Fin de période (obligatoire avec --start)")
    ...
    if args.gravity:
        ts_path, scatter_path = plot_gravity_score()
        print(f"\nTime series : {ts_path.resolve()}")
        print(f"Manifold 2D : {scatter_path.resolve()}")
        return

    if args.date:
        out = map_single_day(date.fromisoformat(args.date))
        ...
        # Heatmap animée (toujours générée)
        out = map_period(start, end)
        print(f"\nHeatmap : {out.resolve()}")

        # Cartes de clusters jour par jour (optionnel)
        if args.each_day:
            d = start
            while d <= end:
                parquet_path = PARQUET_DIR / f"houston_{d.strftime('%Y_%m_%d')}.parquet"
                if parquet_path.exists():
                    try:
                        out_day = map_single_day(d)  # type: ignore[assignment]
                        print(f"  {d} : {out_day.name}")
                    except Exception as exc:
                        log.warning("%s : %s", d, exc)
                else:
                    log.info("%s : pas de parquet, ignoré", d)
                d += timedelta(days=1)
