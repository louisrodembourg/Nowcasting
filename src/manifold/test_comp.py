...
Protocole (Xu et al.) :
  ...
  Test 2 — MSE vs n_components (version Xu et al. stricte) :
    Pour n_comp ∈ {2, 3, ..., 20} composantes :
...
from src.manifold.nmf_pipeline import build_zone_matrix
...
def run_component_test(
    ...
    """
    n_comp_range = n_comp_range or list(range(2, min(31, X.shape[0])))
...
def main() -> None:
    ...
    parser.add_argument("--max-components", type=int, default=30,
                        help="Max components to test for Test 2 (défaut 30)")
    ...
    zone_df, X, dates = build_zone_matrix(start, end, args.location)
    ...
    n_comp_max   = min(args.max_components, min(X.shape) - 1)
