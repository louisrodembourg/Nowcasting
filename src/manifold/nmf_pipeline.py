...
Usage:
    python src/manifold/nmf_pipeline.py --both --start 2019-01-01 --end 2019-01-31 --location houston
"""
...
def identify_constituent_zones(
    ...
    if n_comp < 1:
        raise ValueError(
            f"Not enough data for NMF (zones={X.shape[0]}, days={X.shape[1]})"
        )

    X_pos = X + 1e-10
    model = SklearnNMF(
        n_components=n_comp, init="random", random_state=42, max_iter=1000
    )
    W = model.fit_transform(X_pos)
    reconstruction_error = model.reconstruction_err_
    log.info(f"NMF reconstruction error: {reconstruction_error:.4f}")

    constituents = find_constituent_from_components(W, k=k)
    ...
    return zone_df, reconstruction_error
...
def main():
    parser = argparse.ArgumentParser(description="NMF pipeline for constituent zones")
    ...
    if args.score or args.both:
        ...
        constituent_path = Path(
            f"data/features/{args.location}_nmf_constituent_zones.parquet"
        )
        ...
        fig.update_layout(
            title=dict(
                text=f"{args.location.upper()} - NMF Gravity ({start} to {end})", x=0.5
            ),
            xaxis_title="Date",
            hovermode="x unified",
