...
class ClusteringConfig:
    ...
    # Polygones délimitant les zones docked (terminaux/quais).
    # Tout cluster DANS au moins un de ces polygones → "docked".
    # Tout cluster EN DEHORS → "waiting".
    docked_polygons: Optional[list[list[tuple[float, float]]]] = None
    # Legacy : polygones où le statut "waiting" est possible (logique inverse).
    # Préférer docked_polygons. Si les deux sont définis, docked_polygons prime.
...
def load_waiting_zones(location: str) -> list[list[tuple[float, float]]]:
    """
    Charge les polygones de zone d'attente depuis data/zones/{location}_waiting.geojson.
    Retourne une liste de polygones (chacun = liste de (lat, lon)).
    ...
    path = (
        ...
        / "data"
        / "zones"
        / f"{location}_waiting.geojson"
    )
    return _load_geojson_polygons(path)


def load_docked_zones_or_none(location: str):
    """Charge le polygone docked depuis data/zones/{location}_docked.geojson, None si absent."""
    path = (
        Path(__file__).resolve().parents[2]
        / "data" / "zones" / f"{location}_docked.geojson"
    )
    if not path.exists():
        return None
    try:
...
def _classify_clusters(df: pl.DataFrame, config: ClusteringConfig) -> pl.DataFrame:
    """
    Étiquette chaque cluster comme 'docked' ou 'waiting'.

    Règles de classification (appliquées dans l'ordre) :
      1. docked_polygons défini : cluster DANS au moins un polygone → "docked",
         sinon → "waiting". C'est la logique recommandée (délimite les terminaux).
      2. waiting_allowed_polygons défini (legacy) : cluster DANS → "waiting",
         sinon → "docked".
      3. max_docked_radius_nm défini : cluster compact → "docked", sinon "waiting".
      4. Par défaut : tout cluster → "docked".
    """
    ...
    for label in valid["cluster_label"].unique().to_list():
        ...
        if config.docked_polygons is not None:
            # Docked si au moins un point du cluster est dans un polygone docked.
            any_inside = any(
                _point_in_polygon(lat_i, lon_i, poly)
                for lat_i, lon_i in zip(cluster["LAT"].to_numpy(), cluster["LON"].to_numpy())
                for poly in config.docked_polygons
            )
            is_docked = any_inside

        elif config.waiting_allowed_polygons is not None:
            any_inside = any(
                _point_in_polygon(lat_i, lon_i, poly)
                for lat_i, lon_i in zip(cluster["LAT"].to_numpy(), cluster["LON"].to_numpy())
        ...
        elif config.max_docked_radius_nm is not None:
            lats = cluster["LAT"].to_numpy()
            lons = cluster["LON"].to_numpy()
            is_docked = _compute_cluster_radius_nm(lats, lons) <= config.max_docked_radius_nm

        else:
            is_docked = True

        rows.append({"cluster_label": label, "cluster_type": "docked" if is_docked else "waiting"})
...
def main() -> None:
    ...
    location = parquet_path.parent.name
    ...
    if docked_polys:
        ...
        config = ClusteringConfig(docked_polygons=docked_polys)
    else:
